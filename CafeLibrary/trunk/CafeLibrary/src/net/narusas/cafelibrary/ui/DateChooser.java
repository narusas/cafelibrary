 ...
 */
public class DateChooser extends JDialog implements ItemListener, MouseListener, FocusListener, KeyListener,
		ActionListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = -4903279314097164294L;
	/** Names of the months. */
	private static final String[] MONTHS = new String[] { "1월", "2", "3", "4", "5", "6월", "7월", "8월", "9월", "10월",
			"11월", "12월" };
	...
	private static class FocusablePanel extends JPanel {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -7461790424935056831L;
	...
	private void construct() {
		...
		for (int i = 1; i < 7; i++)
			for (int j = 0; j < 7; j++) {
				...
				if (j == 0) {
					days[i][j].setForeground(Color.RED);
				} else {
	...
	/**
	 * * Gets the selected day, as an <code>int</code>. Parses the text * of
	 * the selected label in the calendar to get the day. * *
	 * 
	...
	/**
	 * * Sets the selected day. The day is specified as the label * control, in
	 * the calendar, corresponding to the day to select. * *
	 * 
	 * @param newDay
	 *            day to select *
	 */
	...
	/**
	 * * Sets the selected day. The day is specified as the number of * the day,
	 * in the month, to selected. The function compute the * corresponding
	 * control to select. * *
	 * 
	 * @param newDay
	 *            day to select *
	...
	/**
	 * * Updates the calendar. This function updates the calendar panel * to
	 * reflect the month and year selected. It keeps the same day * of the month
	 * that was selected, except if it is beyond the last * day of the month. In
	 * this case, the last day of the month is * selected. *
	 */
	private void update() {
	...
	/**
	 * * Called when the "Ok" button is pressed. Just sets a flag and * hides
	 * the dialog box. *
	 */
	...
	/**
	 * * Called when the calendar gains the focus. Just re-sets the * selected
	 * day so that it is redrawn with the border that * indicate focus. *
	 */
	...
	/**
	 * * Called when the calendar loses the focus. Just re-sets the * selected
	 * day so that it is redrawn without the border that * indicate focus. *
	 */
	...
	/**
	 * * Called when a key is pressed and the calendar has the * focus. Handles
	 * the arrow keys so that the user can select a day * using the keyboard. *
	 */
	...
	/**
	 * * Called when the mouse is clicked on a day in the * calendar. Selects
	 * the clicked day. *
	 */
	...
	/**
	 * * Selects new date. Just calls {@link #select(Date)} with the * system
	 * date as the parameter. * *
	 * 
