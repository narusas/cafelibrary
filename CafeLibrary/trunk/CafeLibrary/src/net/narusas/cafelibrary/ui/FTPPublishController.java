		...
		new Thread() {
			...
			public void run() {
				try {
					...
					f.getBookTableControlPanel().getPublishButton().setToolTipText("출판에 성공했습니다");
				} catch (LoginFailException e) {
					e.printStackTrace();
					new Warning(f.getBookTableControlPanel().getPublishButton(), "FTP 로그인에 실패 했습니다.아이디와 암호를 확인해 주세요."
							+ e.getMessage());
				} catch (SocketException e) {
					e.printStackTrace();
					new Warning(f.getBookTableControlPanel().getPublishButton(), "FTP출판에 실패했습니다1." + e.getMessage());
				} catch (FileNotFoundException e) {
					new Warning(f.getBookTableControlPanel().getPublishButton(), "FTP출판에 실패했습니다2." + e.getMessage());
					e.printStackTrace();
				} catch (IllegalStateException e) {
					e.printStackTrace();
					new Warning(f.getBookTableControlPanel().getPublishButton(), "FTP출판에 실패했습니다3." + e.getMessage());
				} catch (IOException e) {
					e.printStackTrace();
					new Warning(f.getBookTableControlPanel().getPublishButton(),
							"FTP 연결에  실패했습니다. 서버주소나 네트웍 연결을 확인해 주세요." + e.getMessage());
				}
